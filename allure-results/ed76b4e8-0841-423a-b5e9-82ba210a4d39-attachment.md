    ...
    - generic [ref=e30]:
      ...
      - generic [ref=e36]:
        ...
        - button "" [ref=e39] [cursor=pointer]:
          - generic [ref=e40]: 
      - generic [ref=e42]:
        - button " 8 item(s) - $2,129.16" [ref=e43] [cursor=pointer]:
          - generic [ref=e44]: 
          - text: 8 item(s) - $2,129.16
        - text:         
  - navigation [ref=e46]:
    - generic: 
    - list [ref=e48]:
...
```ts
  ...
  4  | 
  5  | // 3. Page actions/methods:
  6  | // test@123, test_123@open.com
  7  | 
  8  | /**
  9  |  * this is my dataprovided for product search key
  10 |  */
  11 | let searchData = [
  12 |     {searchKey: 'macbook', resultsCount: 3},
  13 |     {searchKey: 'samsung', resultsCount: 2},
  14 |     {searchKey: 'iMac', resultsCount: 1},
  15 |     {searchKey: 'canon', resultsCount: 1},
  16 |     {searchKey: 'foo', resultsCount: 0},
  17 | ];
  18 | 
  19 | for(let product of searchData){
  20 | test(`verify product search ${product.searchKey}`, async({ page }) => {
  21 | 
  22 |   let loginPage = new LoginPage(page);
  23 |   await loginPage.goToLoginPage();
  24 |   let homePage = await loginPage.doLogin('test_123@open.com', 'test@123');
  25 |   let resultPage: ResultPage = await homePage.doSearch(product.searchKey);
> 26 |   expect(await resultPage.getSearchResultCount()).toBe(product.resultsCount);
     |                                                   ^ Error: expect(received).toBe(expected) // Object.is equality
  27 |  });
  28 | }
  29 | 
